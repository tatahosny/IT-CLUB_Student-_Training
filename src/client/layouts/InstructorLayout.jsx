...
export default function InstructorLayout() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      adminApi.logActivity({
        action: 'page_view',
        description: `Instructor visited ${pageName} page`
      }).catch(() => {});
    }, 500);
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', position: 'relative', background: 'var(--color-bg-primary)' }}>
      <AnimatePresence>{mobileOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 999, backdropFilter: 'blur(4px)' }} />}</AnimatePresence>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflow: 'auto', padding: 24, background: 'var(--color-bg-primary)' }} className="bg-dots main-content">
          <motion.div 
