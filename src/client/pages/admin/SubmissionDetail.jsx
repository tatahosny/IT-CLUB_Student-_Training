          ...
          <div className="glass-card" style={{ padding: 24 }}>
            ...
            {submission?.task?.criteria?.map(c => (
              <div key={c.id} style={{ marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid var(--glass-border)' }}>
                ...
                <input
                  ...
                  value={grades[c.id] || ''}
                  onChange={e => !isHR && setGrades(g => ({ ...g, [c.id]: e.target.value }))}
                ...
                <input
                  type="text"
                  className="input"
                  placeholder={isHR ? "" : "Notes (optional)"}
                  value={notes[c.id] || ''}
                  onChange={e => !isHR && setNotes(n => ({ ...n, [c.id]: e.target.value }))}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea
                className="input"
                rows={3}
                placeholder={isHR ? "" : "Write your feedback to the student here..."}
                value={feedback}
                onChange={e => !isHR && setFeedback(e.target.value)}
            ...
            {isHR && (
              <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', fontSize: 12 }}>
                <Info size={14} style={{ display: 'inline', marginRight: 4 }} /> HR View Mode
              </div>
            )}
          </div>
        </div>
