...
export default function SessionManagement() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => sessionApi.getAll({ limit: 100 }).then(r => r.data.data),
    enabled: !!user?.id,
  ...
  const mySessions = useMemo(() => sessions.filter(s => 
    s.instructors?.some(inst => Number(inst.id) === Number(user?.id))
  ), [sessions, user?.id])
      ...
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800 }}>
            <span className="gradient-text-cyan">إدارة المحاضرات والورش</span>
          </h1>
          <p style={{ color: 'var(--color-text-muted)', fontSize: 14, marginTop: 4 }}>
            {activeSessions.length} جلسات نشطة حالياً • {sessions.length} إجمالي الجلسات المسندة إليك
          </p>
        </div>
        {user.role?.role_name === 'super_admin' && (
          <button className="btn btn-primary" onClick={openCreate}>
            <Plus size={18} /> ورشة عمل جديدة
          </button>
        )}
      </div>
      ...
      <div className="glass-card" style={{ padding: 24 }}>
        ...
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                <th style={{ textAlign: 'right' }}>تفاصيل الجلسة</th>
                <th style={{ textAlign: 'right' }}>النوع</th>
                <th style={{ textAlign: 'right' }}>الوقت والتاريخ</th>
              ...
              {isLoading ? [...Array(3)].map((_, i) => (
                ...
                <tr key={s.id}>
                  <td style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>قاعة: {s.room_number || 'غير محدد'}</div>
                  </td>
                  <td style={{ textAlign: 'right' }}><span className={`badge ${s.session_type === 'workshop' ? 'badge-purple' : 'badge-cyan'}`}>{s.session_type === 'workshop' ? 'ورشة عمل' : 'محاضرة'}</span></td>
                  <td style={{ fontSize: 13, textAlign: 'right' }}>
                    <div>{new Date(s.start_time).toLocaleDateString('ar-EG')}</div>
                    <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>{new Date(s.start_time).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</div>
                  </td>
                  <td style={{ textAlign: 'right' }}>
                    <span className={`badge ${s.is_active ? 'badge-green' : 'badge-gray'}`}>
                      {s.is_active ? 'نشطة الآن' : 'منتهية'}
                    </span>
                  </td>
                  <td>
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                      <button className="btn btn-ghost btn-sm" onClick={() => navigate(`/instructor/sessions/${s.id}/attendance`)} title="تحكم الحضور">
                        <Users size={14} />
                      </button>
                      <button className="btn btn-ghost btn-sm" onClick={() => openEdit(s)} title="تعديل الطاقم">
