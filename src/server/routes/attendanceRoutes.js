...
router.get('/session/:sessionId', roleMiddleware('super_admin', 'instructor', 'mentor_manager', 'mentor', 'hr'), getSessionAttendance);
router.get('/history', getStudentHistory);
router.get('/history/:studentId', roleMiddleware('super_admin', 'instructor'), getStudentHistory);
router.post('/manual', roleMiddleware('super_admin', 'instructor', 'hr'), markAttendanceManual);
