    ...
    <div>
          ...
          {isEditingInfo ? (
            ...
            <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Group:</span>
                <span style={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>{session?.group?.group_name || 'N/A'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Date:</span>
                <span style={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>{session?.start_time ? new Date(session.start_time).toLocaleDateString() : 'N/A'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Time:</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Room:</span>
                <span style={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>{session?.room_number || 'N/A'}</span>
              </div>
            </div>
          )}
            ...
            <div style={{ padding: 12, borderRadius: 12, background: session?.is_active ? 'rgba(34, 197, 94, 0.1)' : 'rgba(255, 255, 255, 0.03)', border: session?.is_active ? '1px solid rgba(34, 197, 94, 0.2)' : '1px solid var(--glass-border)', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>
                  {session?.is_active ? 'Students can scan QR codes' : 'QR codes are disabled'}
                </div>
              </div>
              <button 
              ...
              {searching ? <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Searching...</div> : 
               searchResults?.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {searchResults.map(u => (
                    <button key={u.id} onClick={() => addStaffMutation.mutate(u.id)} className="btn btn-sm" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--color-bg-tertiary)', textAlign: 'left', padding: '6px 10px', width: '100%' }} disabled={addStaffMutation.isPending}>
        ...
        {canGenerateQR && (
          <div className="glass-card" style={{ padding: 20, display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg, rgba(0, 212, 255, 0.05), rgba(124, 58, 237, 0.05))', border: '1px solid rgba(0,212,255,0.2)' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>Student Self-Checkin</div>
                <p style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 4 }}>Set duration and display QR for students.</p>
              </div>
      ...
      <div className="glass-card">
        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ fontSize: 16, fontWeight: 700 }}>Total Students Overview</h3>
        ...
        {showQR && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowQR(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="glass-card" style={{ width: '100%', maxWidth: 500, padding: 40, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
              ...
              <p style={{ color: 'var(--color-text-muted)', marginBottom: 24 }}>Students should scan this to register attendance</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: -8 }}>Remaining Time</div>
      ...
      <ConfirmModal 
        ...
        title="Remove Staff?"
        message="Are you sure you want to remove this staff member from this session?"
      />
    </div>
  )
}
